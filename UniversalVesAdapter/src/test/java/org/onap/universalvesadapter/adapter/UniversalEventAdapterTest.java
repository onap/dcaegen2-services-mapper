...
public class UniversalEventAdapterTest {
  ...
  public void testtransform() {
    ...
    try {
      ...
      PowerMockito.when(CollectorConfigPropertyRetrival.getProperyArray("identifier", "kv.json"))
          .thenReturn(identifiertest);


      /*
       * final Thread mockThread = Mockito.mock(Thread.class);
       * Mockito.when(Thread.currentThread().getName()).thenReturn("ONAP-COLLECTOR-RESTCONFTRAP");
       */

      /*
       * mockStatic(CollectorConfigPropertyRetrival.class);
       * when(CollectorConfigPropertyRetrival.getProperyArray("identifier","")).thenReturn("defused"
       * );
       */
